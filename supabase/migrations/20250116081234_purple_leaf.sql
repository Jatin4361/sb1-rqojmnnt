/*
  # Add payment tables and functions
  ...
  2. Functions
    - `process_payment_success`: Updates user tokens after successful payment
*/
...
BEGIN
  ...
  -- Get plan tokens
  CASE plan_id_input
    WHEN 'silver' THEN plan_tokens := 25;
  ...
  -- Update user tokens
  UPDATE profiles
  SET tokens = current_tokens + plan_tokens
  WHERE id = payment_record.user_id;
