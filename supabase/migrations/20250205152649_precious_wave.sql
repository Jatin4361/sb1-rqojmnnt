/*
  # Fix Razorpay Integration

  1. Changes
    - Add new functions for Razorpay order creation and verification
    - Add better error handling and logging
    - Add proper transaction handling

  2. Security
    - Add proper security checks
    - Add input validation
    - Add proper error messages
*/

-- Drop existing functions if they exist
DROP FUNCTION IF EXISTS create_razorpay_order;
DROP FUNCTION IF EXISTS verify_razorpay_payment;

-- Create function to create Razorpay order with better error handling
CREATE OR REPLACE FUNCTION create_razorpay_order(
  amount_input integer,
  currency_input text,
  plan_id_input text,
  user_id_input uuid
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  order_id text;
  receipt_id text;
  user_exists boolean;
BEGIN
  -- Input validation
  IF amount_input <= 0 THEN
    RETURN json_build_object(
      'success', false,
      'error', 'Invalid amount'
    );
  END IF;

  IF currency_input IS NULL OR currency_input = '' THEN
    currency_input := 'INR';
  END IF;

  -- Check if user exists
  SELECT EXISTS (
    SELECT 1 FROM profiles WHERE id = user_id_input
  ) INTO user_exists;

  IF NOT user_exists THEN
    RETURN json_build_object(
      'success', false,
      'error', 'User not found'
    );
  END IF;

  -- Generate unique IDs
  order_id := 'order_' || substr(md5(random()::text || clock_timestamp()::text), 1, 16);
  receipt_id := 'rcpt_' || substr(md5(random()::text || clock_timestamp()::text), 1, 16);

  -- Start transaction
  BEGIN
    -- Insert payment record
    INSERT INTO payment_history (
      user_id,
      order_id,
      amount,
      plan_id,
      status
    ) VALUES (
      user_id_input,
      order_id,
      amount_input,
      plan_id_input,
      'pending'
    );

    -- Return success response
    RETURN json_build_object(
      'success', true,
      'id', order_id,
      'amount', amount_input,
      'currency', currency_input,
      'receipt', receipt_id
    );
  EXCEPTION WHEN OTHERS THEN
    -- Log error and return failure response
    RAISE NOTICE 'Error creating order: %', SQLERRM;
    RETURN json_build_object(
      'success', false,
      'error', 'Failed to create order: ' || SQLERRM
    );
  END;
END;
$$;

-- Create function to verify Razorpay payment with better error handling
CREATE OR REPLACE FUNCTION verify_razorpay_payment(
  payment_id_input text,
  order_id_input text,
  signature_input text,
  plan_id_input text,
  user_id_input uuid
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  payment_record payment_history;
  plan_config_record plan_config;
  current_tokens integer;
  user_exists boolean;
BEGIN
  -- Input validation
  IF payment_id_input IS NULL OR order_id_input IS NULL OR signature_input IS NULL THEN
    RETURN json_build_object(
      'success', false,
      'error', 'Missing required parameters'
    );
  END IF;

  -- Check if user exists
  SELECT EXISTS (
    SELECT 1 FROM profiles WHERE id = user_id_input
  ) INTO user_exists;

  IF NOT user_exists THEN
    RETURN json_build_object(
      'success', false,
      'error', 'User not found'
    );
  END IF;

  -- Start transaction
  BEGIN
    -- Get payment record with FOR UPDATE to lock the row
    SELECT * INTO payment_record
    FROM payment_history
    WHERE order_id = order_id_input
    AND user_id = user_id_input
    AND status = 'pending'
    FOR UPDATE;

    IF payment_record IS NULL THEN
      RETURN json_build_object(
        'success', false,
        'error', 'Invalid or already processed payment'
      );
    END IF;

    -- Get plan configuration
    SELECT * INTO plan_config_record
    FROM plan_config
    LIMIT 1;

    IF plan_config_record IS NULL THEN
      RETURN json_build_object(
        'success', false,
        'error', 'Plan configuration not found'
      );
    END IF;

    -- Update payment status
    UPDATE payment_history
    SET 
      payment_id = payment_id_input,
      status = 'success',
      updated_at = now()
    WHERE id = payment_record.id;

    -- Get current tokens with FOR UPDATE to lock the row
    SELECT tokens INTO current_tokens
    FROM profiles
    WHERE id = user_id_input
    FOR UPDATE;

    -- Update user tokens and account type
    UPDATE profiles
    SET 
      tokens = current_tokens + plan_config_record.tokens,
      account_type = 'premium',
      updated_at = now()
    WHERE id = user_id_input;

    -- Return success response
    RETURN json_build_object(
      'success', true,
      'user_id', user_id_input,
      'new_tokens', current_tokens + plan_config_record.tokens
    );
  EXCEPTION WHEN OTHERS THEN
    -- Log error and return failure response
    RAISE NOTICE 'Error verifying payment: %', SQLERRM;
    RETURN json_build_object(
      'success', false,
      'error', 'Failed to verify payment: ' || SQLERRM
    );
  END;
END;
$$;

-- Grant execute permissions to authenticated users
GRANT EXECUTE ON FUNCTION create_razorpay_order TO authenticated;
GRANT EXECUTE ON FUNCTION verify_razorpay_payment TO authenticated;